	...
	// This Lab requires you to program a simple game of Tic Tac Toe
		...
		public static void main(String[] args) {
				...
				while(winner.equals("") && allSquares <= 9) {
					...
					System.out.println(player + "s turn. Enter number to place an " + player + " in:");
					...
					if(squareNumber > 0 && squareNumber <= 9) {
						if(gameBoard.ifValidSetSquare(squareNumber, readInput, player)) {
							//change value of player - other player's turn
							if(player.equals("X")) {
								player = "O";
							} else {
								player = "X";
							}
							allSquares++; //increment num of squares
							gameBoard.display();
							winner = gameBoard.checkWinStatus(); //check if there's a winner
							//X, O, draw, or emptyString
						} else { //if not a valid turn
